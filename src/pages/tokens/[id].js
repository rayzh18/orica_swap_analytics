...
function TokenPage() {
  ...
  return (
    <AppShell>
      <Head>
        <title>
          {currencyFormatter.format(price || 0)} | {token.symbol} | Oricaswap
          Analytics
        </title>
      </Head>
      <PageHeader>
        ...
        >
          ...
          <Grid item xs={12} sm="auto" className={classes.links}>
            <Link
              href={`https://oricaswap.org/add/${token.id}/ETH`}
              target="_blank"
              variant="body1"
            >
              Add Liquidity
            ...
            <Link
              href={`https://oricaswap.org/swap?inputCurrency=${token.id}`}
              target="_blank"
              variant="body1"
            >
              Trade
      ...
      <Box my={4}>
        <BasicTable
          ...
          bodyCells={[
            ...
            token.symbol,
            token.id,
            <Link href={`https://bscscan.com/address/${token.id}`}>View</Link>,
          ]}
        />
      </Box>
...
export async function getStaticProps({ params }) {
  ...
  const { data: tokenDayData } = await client.query({
