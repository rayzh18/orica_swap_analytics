...
function PairPage(props) {
  ...
  return (
    <AppShell>
      <Head>
        <title>
          {pair.token0.symbol}-{pair.token1.symbol} | Oricaswap Analytics
        </title>
      </Head>
      <PageHeader>
        <Box display="flex" alignItems="center" className={classes.pageHeader}>
          ...
          <Box display="flex" alignItems="center" className={classes.links}>
            <Link
              href={`https://oricaswap.org/add/${pair.token0.id}/${pair.token1.id}`}
              target="_blank"
              variant="body1"
            ...
            <Link
              href={`https://oricaswap.org/swap?inputCurrency=${pair.token0.id}&outputCurrency=${pair.token1.id}`}
              target="_blank"
              variant="body1"
            >
              Trade
      ...
      <Box my={4}>
        <BasicTable
          ...
          bodyCells={[
            ...
            <Link href={`https://bscscan.com/address/${pair.id}`}>View</Link>,
          ]}
        />
      </Box>
      <Box my={4}>
...
export async function getStaticProps({ params }) {
  ...
  const { data: pairDayData } = await client.query({
  ...
  const { data: trxData } = await client.query({
